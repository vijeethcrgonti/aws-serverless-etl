...
class TestClassifySource:
    def test_known_source(self):
        from lambda.trigger.handler import classify_source
...
class TestValidateFile:
    @patch("lambda.trigger.handler.s3")
    def test_valid_file(self, mock_s3):
        mock_s3.head_object.return_value = {}
        from lambda.trigger.handler import validate_file
...
class TestLambdaHandler:
    @patch("lambda.trigger.handler.trigger_glue_job", return_value="jr_abc123")
    @patch("lambda.trigger.handler.s3")
    @patch("lambda.trigger.handler.glue")
    @patch("lambda.trigger.handler.sns")
    def test_successful_trigger(self, mock_sns, mock_glue, mock_s3, mock_trigger, s3_event):
        mock_s3.head_object.return_value = {}
        from lambda.trigger.handler import lambda_handler
